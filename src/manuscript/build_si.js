...
ch.push(para([B("Definitions (information-theoretic). "),
  "Define prediction-side information I_P(f) = L_P(X, Y, f(X)), the certification-time joint law; a prediction-side certificate is any measurable functional C(f) = Φ(I_P(f)) (covering coverage, accuracy, calibration, Brier, AUC, confidence). Define structural information I_S(f) as information on the internal decision mechanism; the explanation certificate is a functional Ψ(I_S(f)) with fidelity F : S → [0,1]. Prediction-side certification is behavioural; explanation-side is structural."]));
ch.push(para([B("Lemma 1 (non-identifiability). "),I("The map I_P → I_S is not injective: I_P(f) does not determine I_S(f).")," Proof: the construction below exhibits f, f′ with I_P(f) = I_P(f′) but I_S(f) ≠ I_S(f′); hence structural information cannot be recovered from prediction-side information alone. Theorem 1 is the quantitative form. ∎"]));
...
ch.push(para([B("Proof. "),
  "The construction is an existence witness — a single hard instance realising the non-identifiability, in the standard form of an impossibility argument, not a claim that deployed models typically contain such a coordinate; it nonetheless models real mechanisms (backdoor triggers, spurious features rare on the training distribution, dormant sensor channels), and Theorem 2 shows the separation degrades only gracefully when the coordinate is merely near-dormant. Introduce a dormant coordinate v with v(x) = 0 for all x ∈ S = supp(P). Define f with zero weight on v and f′ identical to f except for weight W on v, so s_{f′}(x) = s_f(x) + W·v(x). (i) For x ∈ S, v(x) = 0 gives s_{f′}(x) = s_f(x); hence f and f′ induce the identical joint law L_P(X, Y, f(X)), and every functional C of that law satisfies C(f) = C(f′) exactly. (ii) The sensitivity profiles coincide except in the v-coordinate, where they differ by |W|·E_{ref}|∂_v s|; since the reference measure charges the v-direction, D(φ_f, φ_{f′}) is strictly increasing in |W| and attains any value in [0,1), so choose W with D ≥ β, giving F(f′) = 1 − D ≤ 1 − β. (iii) Let P′ relocate a fraction ρ of the negative-class mass onto points with v = 1; there s_{f′} = s_f + W, and for W large enough these points are classified positive by f′ but not by f, so acc_{P′}(f′) ≤ acc_{P′}(f) − γ for ρ, W chosen to realise the margin γ. ∎"]));
